...
mkseq (intmax_t start, intmax_t end, intmax_t incr, int type, size_t width)
{
  ...
  if (result == 0)
    {
      internal_error (_("brace expansion: failed to allocate memory for %s elements"),
...
expand_seqterm (char *text, size_t tlen)
{
  ...
  /* Now figure out whether LHS and RHS are integers or letters.  Both
     sides have to match. */
  lhs_t = (valid_number (lhs, &tl)) ? ST_INT : ((ISALPHA (lhs[0]) && lhs[1] == 0) ? ST_CHAR : ST_BAD);
...
#  endif
      ...
      if (c == satisfy && level == 0 && quoted == 0 && commas > 0)
	{
	  /* We ignore an open brace surrounded by whitespace, and also
	     ...
	     by whitespace.  */
	  if (c == '{' && ((!i || brace_whitespace (text[i - 1])) && (brace_whitespace (text[i + 1]) || text[i + 1] == '}')))
	    {
	      i++;
	      continue;
	    }
