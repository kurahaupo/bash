...
static void
search (const struct table_entry *table, size_t table_size, const char *string, range_t *result)
{
...
setlocale_unixlike (int category, const char *locale)
{
  ...
  if (strlen (locale) < sizeof (llCC_buf))
	...
	*q = '\0';
	...
	search (language_table, sizeof (language_table) / sizeof (language_table[0]), llCC_buf, &range);
	...
	if (underscore != NULL)
	  {
...

	    {
	      ...
	      search (language_table, sizeof (language_table) / sizeof (language_table[0]), ll_buf, &language_range);
	      if (language_range.lo < language_range.hi)
		{
		  range_t country_range;

		  search (country_table, sizeof (country_table) / sizeof (country_table[0]), CC_buf, &country_range);
  ...
  if (locale != NULL && locale[0] == '\0')
      ...
      if (category == LC_ALL)
	{
	  ...
	  /* Set LC_CTYPE category.  Set all other categories (except possibly
	     ...
	     save calls.  */
	  base_name = gl_locale_name_environ (LC_CTYPE, category_to_name (LC_CTYPE));
	  if (base_name == NULL)
...
#  if defined _WIN32 && ! defined __CYGWIN__
	  ...
	  if (strchr (base_name, '.') != NULL && strcmp (setlocale (LC_CTYPE, NULL), "C") == 0)
	    goto fail;
#  endif
				...
				if (element != NULL
				    ...
				    && CFStringGetCString ((CFStringRef) element, buf, sizeof (buf), kCFStringEncodingASCII))
				  {
				    ...
				    if (setlocale_single (cat, buf) == NULL)
				      {
					const char *last_try = get_main_locale_with_same_language (buf);

					if (last_try == NULL || setlocale_single (cat, last_try) == NULL)
					  warn = 1;
				      }
				  }
			      }
...
#      endif
#    else
			const char *last_try = get_main_locale_with_same_language (name);
...
#    endif
			...
			/* For LC_NUMERIC, the application should use the locale
			   ...
			   territory.  */
			const char *last_try = get_main_locale_with_same_territory (name);
			...
			if (verbose != NULL && verbose[0] != '\0')
			  fprintf (stderr, "Warning: Failed to set locale category %s to %s.\n", category_to_name (cat), name);
		      }
		  }
#  else
		  goto fail;
#  endif
	...
	{
	  char *result;
	  const char *name = gl_locale_name_environ (category, category_to_name (category));
	  if (name == NULL)
	    name = gl_locale_name_default ();
	  ...
	  for (i = 1; i < sizeof (categories) / sizeof (categories[0]); i++)
	    {
	      ...
	      name = gl_locale_name_environ (category, category_to_name (category));
	      if (name == NULL)
		name = gl_locale_name_default ();
