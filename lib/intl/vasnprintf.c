...
divide (mpn_t a, mpn_t b, mpn_t *q)
{
  ...
  if (a_len < b_len)
	...
	for (count = a_len; count > 0; count--)
	  {
	    mp_twolimb_t num = ((mp_twolimb_t) remainder << GMP_LIMB_BITS) | *--sourceptr;
	    *--destptr = num / den;
	    remainder = num % den;
	  }
	...
	for (;;)
	  {
		...
		mp_twolimb_t num = ((mp_twolimb_t) r_ptr[j + b_len] << GMP_LIMB_BITS) | r_ptr[j + b_len - 1];
		q_star = num / b_msd;
		c1 = num % b_msd;
	      }
	    else
		...
		mp_limb_t cr;
		{
		  ...
		  for (count = b_len; count > 0; count--)
		    {
		      /* Here 0 <= carry <= q*.  */
		      carry = carry + (mp_twolimb_t) q_star *(mp_twolimb_t) *sourceptr++ + (mp_limb_t) ~(*destptr);
		...
		if (cr > r_ptr[j + b_len])
		  {
		      ...
		      for (count = b_len; count > 0; count--)
			{
			  ...
			  *destptr++ = source1 + source2 + carry;
			  carry = (carry ? source1 >= (mp_limb_t) ~source2 : source1 > (mp_limb_t) ~source2);
			}
		    }
...
#  endif
    ...
    goto increment_q;
  {
    ...
    for (i = b_len;;)
      {
	mp_limb_t r_i = (i <= r_len && i > 0 ? r_ptr[i - 1] >> (GMP_LIMB_BITS - 1) : 0) | (i < r_len ? r_ptr[i] << 1 : 0);
  ...
  if (c_ptr != NULL)
	...
	*d_ptr++ = '0';
      while (a_len > 0)
	{
	  ...
	  for (count = a_len; count > 0; count--)
	    {
	      mp_twolimb_t num = ((mp_twolimb_t) remainder << GMP_LIMB_BITS) | *--ptr;
	      *ptr = num / 1000000000;
...
MAX_ROOM_NEEDED (const arguments *ap, size_t arg_index, FCHAR_T conversion,
		 arg_type type, int flags, size_t width, int has_precision, size_t precision, int pad_ourselves)
{
  size_t tmp_length;
...
#    if DCHAR_IS_TCHAR
			...
			converted =
			  u8_conv_to_encoding (locale_charset (),
					       iconveh_question_mark, arg, arg_end - arg, NULL, converted, &converted_len);
#    else
			/* Convert from UTF-8 to UTF-16/UTF-32.  */
			converted = U8_TO_DCHAR (arg, arg_end - arg, converted, &converted_len);
#    endif
			if (converted == NULL)
			  {
			    if (!(result == resultbuf || result == NULL))
...
#    else
			...
			converted = U16_TO_DCHAR (arg, arg_end - arg, converted, &converted_len);
#    endif
			if (converted == NULL)
			  {
			    if (!(result == resultbuf || result == NULL))
...
#    else
			...
			converted = U32_TO_DCHAR (arg, arg_end - arg, converted, &converted_len);
#    endif
			if (converted == NULL)
			  {
			    if (!(result == resultbuf || result == NULL))
...
#      endif
		  ...
		  tmpdst =
		    DCHAR_CONV_FROM_ENCODING (locale_charset (),
					      iconveh_question_mark, tmpsrc, characters, NULL, NULL, &tmpdst_len);
		  if (tmpdst == NULL)
		    {
		      free (tmpsrc);
		      if (!(result == resultbuf || result == NULL))
...
#  endif
	    ...
	    else if (dp->conversion == 'c' && a.arg[dp->arg_index].type == TYPE_WIDE_CHAR)
	      {
...
#  if NEED_PRINTF_DIRECTIVE_A || NEED_PRINTF_LONG_DOUBLE
			...
			if (dp->conversion == 'A')
		      ...
		      {
			int sign = 0;
			DECL_LONG_DOUBLE_ROUNDING BEGIN_LONG_DOUBLE_ROUNDING ();
			...
			if (arg > 0.0L && arg + arg == arg)
			...
			else
			  {
			    ...
			    if (has_precision && precision < (unsigned int) ((LDBL_DIG + 1) * 0.831) + 1)
			      {
				/* Round the mantissa.  */
			    ...
			    *p++ = dp->conversion - 'A' + 'X';
			    pad_ptr = p;
			    {
			      ...
			      mantissa -= digit;
			      *p++ = '0' + digit;
			      if ((flags & FLAG_ALT) || mantissa > 0.0L || precision > 0)
				{
				  ...
				  while (mantissa > 0.0L)
				    {
				      ...
				      digit = (int) mantissa;
				      mantissa -= digit;
				      *p++ = digit + (digit < 10 ? '0' : dp->conversion - 10);
				      if (precision > 0)
					precision--;
				    }
				  while (precision > 0)
...
#    if WIDE_CHAR_VERSION
			    {
			      static const wchar_t decimal_format[] = { '%', '+', 'd', '\0' };
...
#  if NEED_PRINTF_DIRECTIVE_A || NEED_PRINTF_DOUBLE
			...
			if (arg > 0.0 && arg + arg == arg)
			...
			else
			  {
			    ...
			    if (has_precision && precision < (unsigned int) ((DBL_DIG + 1) * 0.831) + 1)
			      {
				/* Round the mantissa.  */
			    ...
			    *p++ = dp->conversion - 'A' + 'X';
			    pad_ptr = p;
			    {
			      ...
			      mantissa -= digit;
			      *p++ = '0' + digit;
			      if ((flags & FLAG_ALT) || mantissa > 0.0 || precision > 0)
				{
				  *p++ = decimal_point_char ();
				  /* This loop terminates because we assume
				     that FLT_RADIX is a power of 2.  */
...
#  endif
		...
		length += count;
		      ...
		      || dp->conversion == 'g' || dp->conversion == 'G' || dp->conversion == 'a' || dp->conversion == 'A') && (0
#  if NEED_PRINTF_DOUBLE
															       ||
															       a.arg
															       [dp->arg_index].type
...
#    if NEED_PRINTF_LONG_DOUBLE
				...
				digits = scale10_round_decimal_long_double (arg, precision);
				if (digits == NULL)
				  {
				    END_LONG_DOUBLE_ROUNDING ();
				    goto out_of_memory;
				...
				if (arg == 0.0L)
				...
				else
				  {
				    ...
				    for (;;)
				      {
					digits = scale10_round_decimal_long_double (arg, (int) precision - exponent);
					if (digits == NULL)
					  {
					    END_LONG_DOUBLE_ROUNDING ();
					    goto out_of_memory;
					...
					if (ndigits == precision + 1)
					  break;
					if (ndigits < precision || ndigits > precision + 2)
					...
					char *digits2 = scale10_round_decimal_long_double (arg,
...
#      if WIDE_CHAR_VERSION
				{
				  static const wchar_t decimal_format[] = { '%', '+', '.', '2', 'd', '\0' };
				  SNPRINTF (p, 6 + 1, decimal_format, exponent);
				}
				while (*p != '\0')
				  p++;
				...
				if (arg == 0.0L)
				     ...
				     Use fixed-point notation.  */
				  {
				    ...
				    /* Number of trailing zeroes that have to be
				       dropped.  */
				    size_t nzeroes = (flags & FLAG_ALT ? 0 : precision - 1);
				...
				else
				  {
				    ...
				    for (;;)
				      {
					digits = scale10_round_decimal_long_double (arg, (int) (precision - 1) - exponent);
					if (digits == NULL)
					  {
					    END_LONG_DOUBLE_ROUNDING ();
					    goto out_of_memory;
					...
					if (ndigits == precision)
					  break;
					if (ndigits < precision - 1 || ndigits > precision + 1)
					  /* The exponent was not guessed
					     precisely enough.  */
					  abort ();
					if (adjusted)
				    ...
				    nzeroes = 0;
				    if ((flags & FLAG_ALT) == 0)
				      while (nzeroes < ndigits && digits[nzeroes] == '0')
					nzeroes++;
...
#    else
				...
				*p++ = '0';
				if (flags & FLAG_ALT)
				  {
				    size_t ndigits = (precision > 0 ? precision - 1 : 0);
				    *p++ = decimal_point_char ();
...
#    if NEED_PRINTF_DOUBLE
				...
				digits = scale10_round_decimal_double (arg, precision);
				if (digits == NULL)
				  goto out_of_memory;
				ndigits = strlen (digits);
				...
				if (arg == 0.0)
				...
				else
				  {
				    ...
				    for (;;)
				      {
					digits = scale10_round_decimal_double (arg, (int) precision - exponent);
					if (digits == NULL)
					  goto out_of_memory;
					ndigits = strlen (digits);
					...
					/* Maybe the exponent guess was too high
					   and a smaller exponent can be reached
					   by turning a 10...0 into 9...9x.  */
					char *digits2 = scale10_round_decimal_double (arg,
										      (int) precision - exponent + 1);
					if (digits2 == NULL)
					  {
					    free (digits);
...
#      endif
				...
				if (arg == 0.0)
				...
				else
				  {
				    ...
				    for (;;)
				      {
					digits = scale10_round_decimal_double (arg, (int) (precision - 1) - exponent);
					if (digits == NULL)
					  goto out_of_memory;
					ndigits = strlen (digits);
...
#  endif
		...
		tmp_length =
		  MAX_ROOM_NEEDED (&a, dp->arg_index, dp->conversion, type,
				   flags, width, has_precision, precision, pad_ourselves);
...
#  define TCHARS_PER_DCHAR (sizeof (DCHAR_T) / sizeof (TCHAR_T))
		...
		ENSURE_ALLOCATION (xsum (length, (2 + TCHARS_PER_DCHAR - 1) / TCHARS_PER_DCHAR));
...
#  define SNPRINTF_BUF(arg) \
			...
			/* Verify that snprintf() has NUL-terminated its
			   result.  */
			if ((unsigned int) count < maxlen && ((TCHAR_T *) (result + length))[count] != '\0')
			  abort ();
			/* Portability hack.  */
			if (retcount > count)
			  count = retcount;
...
#  if !HAVE_SNPRINTF_RETVAL_C99 || USE_MSVC__SNPRINTF
				...
				size_t tmp_length = MAX_ROOM_NEEDED (&a, dp->arg_index,
				...
				if (maxlen < tmp_length)
				  {
				    ...
				    size_t bigger_need = xsum (length,
							       xsum (tmp_length,
								     TCHARS_PER_DCHAR - 1) / TCHARS_PER_DCHAR);
				    /* And always grow proportionally.
				       (There may be several arguments, each
				       needing a little more room than the
				       previous one.)  */
				    size_t bigger_need2 = xsum (xtimes (allocated, 2), 12);
			...
			if (maxlen == INT_MAX / TCHARS_PER_DCHAR)
			...
			else
			  {
			    ...
			    size_t n = xmax (xsum (length,
						   ((unsigned int) count + 2 + TCHARS_PER_DCHAR - 1) / TCHARS_PER_DCHAR),
					     xtimes (allocated, 2));
...
#  endif
			...
			if (count >= 1 && (*prec_ptr == '-' || *prec_ptr == '+' || *prec_ptr == ' '))
			  prefix_count = 1;
			/* Put the additional zeroes after the 0x prefix if
			   (flags & FLAG_ALT) || (dp->conversion == 'p').  */
			else if (count >= 2 && prec_ptr[0] == '0' && (prec_ptr[1] == 'x' || prec_ptr[1] == 'X'))
			  prefix_count = 2;
...
#  if USE_SNPRINTF
			    size_t n = xsum (length,
					     (count + insert + TCHARS_PER_DCHAR - 1) / TCHARS_PER_DCHAR);
...
#  endif
			...
			/* Need at least count elements.  But allocate
			   proportionally.  */
			size_t n = xmax (xsum (length, count), xtimes (allocated, 2));
...
#  endif
				{
				  ...
				  if ((*pad_ptr >= 'A' && *pad_ptr <= 'Z') || (*pad_ptr >= 'a' && *pad_ptr <= 'z'))
				    pad_ptr = NULL;
				}
