...
enum_locales_fn (LPSTR locale_num_str)
{
  ...
  if (GetLocaleInfo (try_lcid, LOCALE_SENGLANGUAGE, locval, LOCALE_NAME_MAX_LENGTH))
    {
      strcat (locval, "_");
      if (GetLocaleInfo (try_lcid, LOCALE_SENGCOUNTRY, locval + strlen (locval), LOCALE_NAME_MAX_LENGTH))
	{
	  size_t locval_len = strlen (locval);

	  if (strncmp (locval, lname, locval_len) == 0 && (lname[locval_len] == '.' || lname[locval_len] == '\0'))
	    {
	      found_lcid = try_lcid;
	      return FALSE;
	    }
     ...
     static const char *struniq (const char *string)
{
  ...
  new_node = (struct struniq_hash_node *) malloc (FLEXSIZEOF (struct struniq_hash_node, contents, size));
  if (new_node == NULL)
...
#  undef newlocale
{
  ...
  if (((LC_CTYPE_MASK | LC_NUMERIC_MASK | LC_TIME_MASK | LC_COLLATE_MASK
	| LC_MONETARY_MASK | LC_MESSAGES_MASK) & category_mask) != 0)
    name = struniq (name);
  ...
  if (((LC_CTYPE_MASK | LC_NUMERIC_MASK | LC_TIME_MASK | LC_COLLATE_MASK
      ...
      if (base == NULL)
	{
	  ...
	  for (category = 0; category < 6; category++)
		...
		default:
	      ...
	      names.category_name[category] = ((mask & category_mask) != 0 ? name : "C");
	    }
	}
      else if (base == LC_GLOBAL_LOCALE)
	{
	  ...
	  for (category = 0; category < 6; category++)
		...
		((mask & category_mask) != 0 ? name : get_locale_t_name (category, LC_GLOBAL_LOCALE));
	    }
	}
      else
	{
	  ...
	  /* Lock while looking up the hash node.  */
	  gl_rwlock_rdlock (locale_lock);
	  for (p = locale_hash_table[locale_hash_function (base) % LOCALE_HASH_TABLE_SIZE]; p != NULL; p = p->next)
	    if (p->locale == base)
	      break;
	  ...
	  for (category = 0; category < 6; category++)
		...
		default:
	      ...
	      names.category_name[category] =
		((mask & category_mask) != 0 ? name : (p != NULL ? p->names.category_name[category] : ""));
	    }
  ...
  if (locale == LC_GLOBAL_LOCALE)
    {
      ...
      for (category = 0; category < 6; category++)
	node->names.category_name[category] = get_locale_t_name (category, LC_GLOBAL_LOCALE);
  ...
  else
    {
      ...
      for (p = locale_hash_table[locale_hash_function (locale) % LOCALE_HASH_TABLE_SIZE]; p != NULL; p = p->next)
	if (p->locale == locale)
	  break;
      if (p != NULL)
	node->names = p->names;
