...
static void
morecore (register int nu)		/* size index to get more of  */
{
  register union mhead *mp;
  register int nblks;
  register long siz;
...
static PTR_T
internal_malloc (size_t n, const char *file, int line, int flags) /* get a block */
{
...
static PTR_T
internal_realloc (PTR_T mem, register size_t n, const char *file, int line, int flags)
{
  register union mhead *p;
