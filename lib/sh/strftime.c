...
#  endif
	...
	case 'v':		/* date as dd-bbb-YYYY */
	  sprintf (tbuf, "%2d-%3.3s-%4ld",
		   range (1, timeptr->tm_mday, 31), months_a (range (0, timeptr->tm_mon, 11)), timeptr->tm_year + 1900L);
	  for (i = 3; i < 6; i++)
	    if (islower (tbuf[i]))
...
Date: Wed, 24 Apr 91 20: 54: 08 MDT From: Michal Jaegermann < audfax ! emory ! vm.ucs.UAlberta.CA ! NTOMCZAK > To:arnold @ audiofax.com
  Hi Arnold,
  in a process of fixing of strftime ()
     in libraries on Atari ST
       I
       grabbed
       some
       pieces
 ...
 */
     {
       return (timeptr->tm_yday - timeptr->tm_wday + (firstweekday ? (timeptr->tm_wday ? 8 : 1) : 7)) /
	 7;
     }
     How nicer it depends on
       a
       compiler,
       of
       course,
