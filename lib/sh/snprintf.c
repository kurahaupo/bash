...
#    endif
    ((p->flags & PF_SPACE) ? 1 : 0) - strlen (tmp) - p->precision - ((p->precision != 0 || (p->flags & PF_ALTFORM)) ? 1 : 0); /* radix char */
...
#    else
    ((d > 0. && (p->flags & PF_PLUS)) ? 1 : 0) -
#    endif
    (p->precision != 0 || (p->flags & PF_ALTFORM)) - ((p->flags & PF_SPACE) ? 1 : 0) - p->precision - 5;
