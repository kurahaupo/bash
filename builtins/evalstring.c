...
should_optimize_fork (COMMAND *command, int subshell)
{
  return (running_trap == 0 &&
	  ...
	  ((command->flags & CMD_TIME_PIPELINE) == 0) && ((command->flags & CMD_INVERT_RETURN) == 0));
}
...
should_suppress_fork (COMMAND *command)
{
  ...
  return (startup_state == 2 && parse_and_execute_level == 1 &&
	  *bash_input.location.string == '\0' &&
	  ...
	  job_control_active_p () == 0 && ssh_reading_startup_files == 0 && should_optimize_fork (command, subshell));
}
...
can_optimize_connection (COMMAND *command)
{
  return ((bash_input.type != st_string || *bash_input.location.string == '\0') &&
	  ...
	  (command->value.Connection->connector == AND_AND || command->value.Connection->connector == OR_OR
	   || command->value.Connection->connector == ';') && command->value.Connection->second->type == cm_simple);
}
...
optimize_connection_fork (COMMAND *command)
{
  if (command->type == cm_connection &&
      (command->value.Connection->connector == AND_AND || command->value.Connection->connector == OR_OR
       || command->value.Connection->connector == ';') && (command->value.Connection->second->flags & CMD_TRY_OPTIMIZING)
      ...
      if (parse_command () == 0)
	{
	  ...
	  if ((flags & SEVAL_PARSEONLY) || (interactive_shell == 0 && read_but_dont_execute))
	      ...
	      if (should_suppress_fork (command))
		{
	      ...
	      else if (command->type == cm_connection && (flags & SEVAL_NOOPTIMIZE) == 0 && can_optimize_connection (command))
		{
		  command->value.Connection->second->flags |= CMD_TRY_OPTIMIZING;
		  command->value.Connection->second->value.Simple->flags |= CMD_TRY_OPTIMIZING;
		}
