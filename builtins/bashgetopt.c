...
static inline _Bool
NOTOPT (char const *s, _Bool plus)
{
  return !((s[0] == '-' || s[0] == '+' && plus) && s[1]);
}
...
internal_getopt (WORD_LIST *list, char *opts)
{
  ...
  if (sp == 1)
    {
      if (lcurrent == 0 || NOTOPT (lcurrent->word->word, plus))
	{
  ...
  if (*++cp == ':' || *cp == ';')
      ...
      else if (lcurrent->next && (*cp == ':' || lcurrent->next->word->word[0] != '-'))
	{
      ...
      else if (lcurrent->next && (*cp == ':' || NOTOPT (lcurrent->next->word->word, plus)))
	{
#endif
	  lcurrent = lcurrent->next;
