...
add_links (char *c)
{
  ...
  while (nr)
	  ...
	  if (g && f - g < 6 && (isalnum (f[-1]) || f[-1] == '>') &&
	      ...
	      if (isalnum (*h))
		{
		  ...
		  if ((subsec == 'X' && f[3] != ')') || subsec == ')')
		    subsec = '\0';
		  while (h > c && (isalnum (h[-1]) || h[-1] == '_' || h[-1] == '-' || h[-1] == '.'))
		    h--;
		  t = *h;
		  *h = '\0';
		  fputs (c, stdout);
		  ...
		  *e = '\0';
		  if (subsec)
		    printf ("<A HREF=\"" CGIBASE "?man%c/%s.%c%c\">%s</A>", sec, h, sec, tolower (subsec), h);
		  else
		    printf ("<A HREF=\"" CGIBASE "?man%c/%s.%c\">%s</A>", sec, h, sec, h);
		  *e = t;
		  c = e;
		}
	    }
	...
	case 1:		/* mailto */
	  g = f = idtest[1];
	  while (g > c && (isalnum (g[-1]) || g[-1] == '_' || g[-1] == '-' || g[-1] == '+' || g[-1] == '.' || g[-1] == '%'))
	    g--;
	  h = f + 1;
...
scan_request (char *c)
{
  ...
  if (c[0] == escapesym)
	...
	default:
	  ...
	  else if (mandoc_command && ((isupper (*c) && islower (*(c + 1))) || (islower (*c) && isupper (*(c + 1)))))
...
#define FLUSHIBP  if (ibp) { intbuff[ibp]=0; out_html(intbuff); ibp=0; }
  ...
  while (*h && (!san || newline_for_fun || *h != '\n'))
      ...
      else if (*h == controlsym && h[-1] == '\n')
	{
      ...
      else if (mandoc_line && *(h) && isupper (*(h)) && *(h + 1) && islower (*(h + 1)) && *(h + 2) && isspace (*(h + 2)))
	{
	  /*
