...
{
  return (ae ? array_create_element (element_index (ae), element_value (ae)) : (ARRAY_ELEMENT *) NULL);
}
#  endif
...
array_to_kvpair (ARRAY *a, int quoted)
{
  ...
  for (ae = element_forw (a->head); ae != a->head; ae = element_forw (ae))
	...
	(ansic_shouldquote (element_value (ae)) ?
	 ansic_quote (element_value (ae), 0, (int *) 0) : sh_double_quote (element_value (ae))) : (char *) NULL;
