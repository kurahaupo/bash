     ...
     int mm, dd, yy, h, m, s, mer, zone, dayflag;
{
  ...
  if (yy < epoch || yy > 1999 || mm < 1 || mm > 12 || dd < 1 || dd > mdays[--mm])
    return (-1);
  jdate = dd - 1;
  for (i = 0; i < mm; i++)
    jdate += mdays[i];
     ...
     time_t sdate, relmonth;
{
  ...
  yy = mm / 12;
  mm = mm % 12 + 1;
  return daylcorr (dateconv (mm, ltime->tm_mday, yy, ltime->tm_hour, ltime->tm_min, ltime->tm_sec, 24, ourzone, MAYBE), sdate);
}
...
#define mcheck(f)	if (f>1) err++
  ...
  if (dateflag || timeflag || dayflag)
  ...
  else
    {
      sdate = now->time;
      if (relflag == 0)
	sdate -= (lt->tm_sec + lt->tm_min * 60 + lt->tm_hour * (60L * 60L));
    }
