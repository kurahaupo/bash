...
static int asynchronous_notification = 0;
...
static opt_def_t const OPTDEF_jobs_m_flag = {
  ...
  .hide_shopt = true,
  .help = N_(
    "Enable job control, and report when jobs are stopped."),
};
  ...
  for (job = 0, dir = NULL; job < js.j_jobslots; job++)
      ...
      if (jobs[job] && IS_NOTIFIED (job) == 0)
	{
	  ...
	  switch (JOBSTATE (job))
		...
		{
		  internal_debug ("notify_of_job_status: catch-all setting J_NOTIFIED on job %d (%d), startup state = %d subshell_environment = %d",
				  job, jobs[job]->flags, startup_state, subshell_environment);
		  jobs[job]->flags |= J_NOTIFIED;
		}
	      break;
