...
int
discard_pipeline (register PROCESS *chain)
{
...
static PROCESS *
find_pipeline (pid_t pid, int alive_only, int *jobp)		/* index into jobs list or NO_JOB */
{
  int job;
  PROCESS *p;
...
/* Return the PROCESS * describing PID.  If JOBP is non-null return the index
   ...
   SIGCHLD blocked. */
static PROCESS *
find_process (pid_t pid, int alive_only, int *jobp)		/* index into jobs list or NO_JOB */
{
  PROCESS *p;
