...
static opt_test_func_t *const hide_unless_map[] = {
  ...
  [AC_env_bashopts] = hide_unless_env_bashopts,
  [AC_env_shellopts] = hide_unless_env_shellopts,
};

/* not static */
inline opt_test_func_t *
hidden_check_for (accessor_t why)
{
  int x = AccessorC (why);
  if (x < 0 || x >= sizeof hide_unless_map / sizeof *hide_unless_map)
    return NULL;
  return hide_unless_map[x];
}
...
{
  return count_options_func (hidden_check_for (why));
}
...
const option_value_t OPTINIT_0[] = {0};
const option_value_t OPTINIT_1[] = {1};
